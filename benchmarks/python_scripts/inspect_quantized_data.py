...
def generate_adsampling_ivf(dataset_name: str, _types=('pdx', 'dual')):
    ...
    for list_id in range(5):
        ...
        ads_extended_data = np.vstack([ads_pre_data, ads_means, ads_stdevs, ads_max_vals, ads_min_vals, ads_ranges, ads_p90])
        np.savetxt(f'./benchmarks/python_scripts/partition_info/{dataset_name}_{list_id}.dat', extended_data, '%6.3f')
        np.savetxt(f'./benchmarks/python_scripts/partition_info/ads_{dataset_name}_{list_id}.dat', ads_extended_data, '%6.3f')
    # PDX
