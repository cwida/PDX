...
def generate_adsampling_ivf(dataset_name: str, _types=('pdx', 'dual')):
    ...
    for list_id in range(5):
        ...
        if list_id == 0:
            np.savetxt(f'./benchmarks/python_scripts/partition_info/ads_t{dataset_name}_{list_id}.dat', for_data, '%6.3f')
            np.savetxt(f'./benchmarks/python_scripts/partition_info/ads_extended_{dataset_name}_{list_id}.dat', ads_extended_data, '%6.3f')
            np.savetxt(f'./benchmarks/python_scripts/partition_info/{dataset_name}_{list_id}.dat', pre_data, '%6.3f')
    # PDX
