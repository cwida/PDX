...
if __name__ == '__main__':
    RESULTS_PATH = os.path.join(RESULTS_DIRECTORY, "IVF_LORANN.csv")
    arg_dataset = ""
    IVF_NPROBE = 0
    if len(sys.argv) > 1:
    ...
    for dataset in DATASETS_TO_USE:
        ...
        index_name = os.path.join(CORE_INDEXES_LORANN, get_core_index_filename(dataset))
        gt_name = os.path.join(SEMANTIC_GROUND_TRUTH_PATH, get_ground_truth_filename(dataset, 100))

        if BUILD:
            print('Building LoRANN index for', dataset)
            print('Loading data')
            ...
            print('Saving')
            index.save(str(index_name))
            continue

        queries = read_hdf5_test_data(dataset)
        queries = preprocessing.normalize(queries, axis=1, norm='l2')

        print('Restoring Lorann index...')
        index = lorann.LorannIndex.load(str(index_name))
        ...
        for ivf_nprobe in nprobes_to_use:
            ...
            if IVF_NPROBE > 0 and IVF_NPROBE != ivf_nprobe:
                continue
            if ivf_nprobe > index.n_clusters:
                continue
            runtimes = []
            recalls = []
            clock = TicToc()
            ...
            for i in range(N_MEASURE_RUNS):
                for q in queries:
                    ...
                    clock.tic()
                    index.search(q, KNN, clusters_to_search=ivf_nprobe, points_to_rerank=200, return_distances=False, n_threads=1)
            ...
            query_i = 0
            for q in queries:
                matches = index.search(np.ascontiguousarray(np.array([q])), KNN, clusters_to_search=ivf_nprobe, points_to_rerank=200, return_distances=False, n_threads=-1)
