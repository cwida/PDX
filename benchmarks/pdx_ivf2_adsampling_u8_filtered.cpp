...
int main(int argc, char* argv[]) {
    ...
    for (const auto& dataset : BenchmarkUtils::DATASETS) {
        ...
        std::unique_ptr<char[]> _matrix_ptr =
            MmapFile(BenchmarkUtils::PDX_ADSAMPLING_DATA + dataset + "-ivf2-u8-matrix");
        ...
        PDX::ADSamplingPruner pruner =
            PDX::ADSamplingPruner<PDX::U8>(pdx_data.num_dimensions, EPSILON0, _matrix);
        PDX::PDXearch searcher = PDX::PDXearch<PDX::U8, PDX::IndexPDXIVF2<PDX::U8>>(
            pdx_data, pruner, 1, DIMENSION_ORDER
        );
        ...
        for (size_t ivf_nprobe : nprobes_to_use) {
            ...
            if (VERIFY_RESULTS) {
                for (size_t l = 0; l < NUM_QUERIES; ++l) {
                    ...
                    BenchmarkUtils::VerifyResult<true, PDX::U8>(
