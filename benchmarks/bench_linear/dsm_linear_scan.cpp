...
int main(int argc, char *argv[]) {
    ...
    std::string RESULTS_PATH = BENCHMARK_UTILS.RESULTS_DIR_PATH + "EXACT_DSM.csv";
    ...
    for (const auto & dataset : BenchmarkUtils::DATASETS) {
        ...
        if (VERIFY_RESULTS){
            for (size_t l = 0; l < NUM_QUERIES; ++l) {
                auto result = searcher.LinearScanDSM(query + l * pdx_data.num_dimensions, KNN);
                BenchmarkUtils::VerifyResult<true>(recalls, result, KNN, int_ground_truth, l);
            }
        }
        for (size_t j = 0; j < NUM_MEASURE_RUNS; ++j) {
            for (size_t l = 0; l < NUM_QUERIES; ++l) {
                searcher.LinearScanDSM(query + l * pdx_data.num_dimensions, KNN);
