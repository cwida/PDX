...
int main(int argc, char *argv[]) {
    ...
    float * data;
    size_t skipping_size = DIMENSION * 32;
    size_t vector_chunks = N_VECTORS / 32;
    for (size_t j = 0; j < NUM_WARMUP_RUNS; ++j) {
        data = raw_data;
        for (size_t vector_chunk = 0; vector_chunk < vector_chunks; ++vector_chunk){
            PDXScanner<PDX::L2, 32>::ResetDistances();
            PDXScanner<PDX::L2, 32>::CalculateVerticalDistancesVectorized(query, data, DIMENSION);
            data += skipping_size;
        }
    }
    std::cout << std::setprecision(16) << PDXScanner<PDX::L2, 32>::distances[32 - 1] << "\n";
    std::cout << std::setprecision(16) << PDXScanner<PDX::L2, 32>::distances[0] << "\n";
