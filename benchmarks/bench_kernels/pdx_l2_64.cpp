...
int main(int argc, char *argv[]) {
    ...
    float * data;
    size_t skipping_size = DIMENSION * 64;
    size_t vector_chunks = N_VECTORS / 64;

    for (size_t j = 0; j < NUM_WARMUP_RUNS; ++j) {
        data = raw_data;
        for (size_t vector_chunk = 0; vector_chunk < vector_chunks; ++vector_chunk){
            PDXScanner<PDX::L2, 64>::ResetDistances();
            PDXScanner<PDX::L2, 64>::CalculateVerticalDistancesVectorized(query, data, DIMENSION);
            data += skipping_size;
        }
    }
    std::cout << std::setprecision(16) << PDXScanner<PDX::L2, 64>::distances[64 - 1] << "\n";
