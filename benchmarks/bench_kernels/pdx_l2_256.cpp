...
int main(int argc, char *argv[]) {
    ...
    float * data;
    size_t skipping_size = DIMENSION * 256;
    size_t vector_chunks = N_VECTORS / 256;

    for (size_t j = 0; j < NUM_WARMUP_RUNS; ++j) {
        data = raw_data;
        for (size_t vector_chunk = 0; vector_chunk < vector_chunks; ++vector_chunk){
            PDXScanner<PDX::L2, 256>::ResetDistances();
            PDXScanner<PDX::L2, 256>::CalculateVerticalDistancesVectorized(query, data, DIMENSION);
            data += skipping_size;
        }
    }
    std::cout << std::setprecision(16) << PDXScanner<PDX::L2, 256>::distances[256 - 1] << "\n";
