...
int main(int argc, char *argv[]) {
    ...
    float * data;
    size_t skipping_size = DIMENSION * 128;
    size_t vector_chunks = N_VECTORS / 128;

    for (size_t j = 0; j < NUM_WARMUP_RUNS; ++j) {
        data = raw_data;
        for (size_t vector_chunk = 0; vector_chunk < vector_chunks; ++vector_chunk){
            PDXScanner<PDX::L2, 128>::ResetDistances();
            PDXScanner<PDX::L2, 128>::CalculateVerticalDistancesVectorized(query, data, DIMENSION);
            data += skipping_size;
        }
    }
    std::cout << std::setprecision(16) << PDXScanner<PDX::L2, 128>::distances[128 - 1] << "\n";
