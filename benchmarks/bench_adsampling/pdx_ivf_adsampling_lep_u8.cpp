...
int main(int argc, char *argv[]) {
    ...
    for (const auto & dataset : BenchmarkUtils::DATASETS) {
        ...
        PDX::IndexPDXIVF pdx_data = PDX::IndexPDXIVF<PDX::Quantization::ASYMMETRIC_LEP_U8>();
        pdx_data.Restore(BenchmarkUtils::PDX_ADSAMPLING_DATA + dataset + "-lep-ivf");
        float * _matrix = MmapFile32(BenchmarkUtils::NARY_ADSAMPLING_DATA + dataset + "-lep-matrix");
...
//        std::cout << (1.0 * tt.accum_time / 1e6) / 1000 << "miliseconds per query\n";
        ...
        PDX::ADSamplingSearcher searcher = PDX::ADSamplingSearcher<PDX::ASYMMETRIC_LEP_U8>(pdx_data, 1, EPSILON0, matrix, DIMENSION_ORDER);
        ...
        for (size_t ivf_nprobe : BenchmarkUtils::IVF_PROBES) {
            ...
            if (VERIFY_RESULTS) {
                for (size_t l = 0; l < NUM_QUERIES; ++l) {
                    ...
                    BenchmarkUtils::VerifyResult<true, PDX::ASYMMETRIC_LEP_U8>(recalls, result, KNN, int_ground_truth, l);
                }
            }
