...
#ifndef BENCHMARK_PHASES
#define BENCHMARK_PHASES = true
#endif
...
int main(int argc, char *argv[]) {
    ...
    std::string RESULTS_PATH = BENCHMARK_UTILS.RESULTS_DIR_PATH + "PHASES_IVF_NARY_ADSAMPLING_SIMD.csv";
    ...
    for (const auto & dataset : BenchmarkUtils::DATASETS) {
        ...
        int DELTA_D = 32;
        float *nary_data = MmapFile32(BenchmarkUtils::NARY_ADSAMPLING_DATA + dataset + "-ivf-dual-block");
        ...
        uint32_t num_dimensions = ((uint32_t *) nary_data)[0];
        nary_data += 1;
        uint32_t num_embeddings = ((uint32_t *) nary_data)[0];
        nary_data += 1;
        ...
        for (size_t ivf_nprobe : BenchmarkUtils::IVF_PROBES_PHASES) {
            ...
            for (size_t j = 0; j < NUM_MEASURE_RUNS; ++j) {
                for (size_t l = 0; l < NUM_QUERIES; ++l) {
                    ...
                    runtimes[j + l * NUM_MEASURE_RUNS] = {
                            ...
                            searcher.distance_calculation.accum_time
                    };
                }
            }
