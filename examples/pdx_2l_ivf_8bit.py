...
from pdxearch import IndexPDXIVF2SQ8

np.random.seed(42)

"""
PDXearch with a two-level IVF index + 8-bit scalar quantization (U8).
...
if __name__ == "__main__":
    ...
    nprobe = 25
    knn = 100
    print(f'Running example: PDXearch Two-Level IVF (SQ8)\n- D={num_dimensions}\n- k={knn}\n- nprobe={nprobe}\n- dataset={dataset_name}')
    train, queries = read_hdf5_data(os.path.join('./benchmarks/datasets/downloaded', dataset_name))

    index = IndexPDXIVF2SQ8(num_dimensions=num_dimensions, normalize=True)
