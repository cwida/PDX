...
We benchmarked several algorithms in different data layouts using C++. For this, we build and store one index for each benchmarked algorithm. For instance, we create one file with the IVF index and the vectors in the PDX layout for ADSampling and another with the same IVF index but the vectors in the N-ary layout for ADSampling.
...
- `/benchmarks/benchmark_kernels.sh`: `make` and runs benchmarks for the PDX vs SIMD kernels (Section 6.2 of our paper).
