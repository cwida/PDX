...
#define PDX_CONCAT_IMPL(x, y) x##y
#define PDX_CONCAT(x, y) PDX_CONCAT_IMPL(x, y)

// Profiling macros - only enabled when BENCHMARK_TIME is defined
#ifdef BENCHMARK_TIME
// PDX_PROFILE_SCOPE creates a scoped timer with the given name
#define PDX_PROFILE_SCOPE(name) ::PDX::ScopedTimer PDX_CONCAT(_pdx_timer_, __LINE__)(name)

// PDX_PROFILE_FUNCTION creates a scoped timer with the function name
#define PDX_PROFILE_FUNCTION() PDX_PROFILE_SCOPE(__func__)

// Manual start/stop macros
#define PDX_PROFILE_START(name) ::PDX::Profiler::Get().Start(name)
#define PDX_PROFILE_STOP(name) ::PDX::Profiler::Get().Stop(name)
#else
// No-op macros when profiling is disabled
#define PDX_PROFILE_SCOPE(name) ((void) 0)
#define PDX_PROFILE_FUNCTION() ((void) 0)
#define PDX_PROFILE_START(name) ((void) 0)
#define PDX_PROFILE_STOP(name) ((void) 0)
#endif
